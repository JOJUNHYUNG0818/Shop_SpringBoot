...
public class ItemController {
	...
	public String itemForm(Model model) { //이부분도 수정
		model.addAttribute("itemFormDto", new ItemFormDto() ); //이부분 수정
	...
	public String itemNew(@Valid ItemFormDto itemFormDto, BindingResult bindingResult, Model model, @RequestParam("itemImgFile") List<MultipartFile> itemImgFileList ) {       // 
		...
		try {
			...
			System.out.println(itemImgFileList.get(0)+"********************");
			Long id = itemService.saveItem(itemFormDto, itemImgFileList);
		} catch (Exception e) {
			model.addAttribute("errorMessage","상품 등록 중 에러가 발생하였습니다");
			return "item/itemForm";
		}
		return "redirect:/"; // 루트로 (메인)
	}
	...
	public String itemUpdate(@Valid ItemFormDto itemFormDto,
		...
		try {
			...
			model.addAttribute("errorMessage","상품 수정 중 에러가 밸상했습니다.");
			return "item/itemForm";
		}
		return "redirect:/";
	}
